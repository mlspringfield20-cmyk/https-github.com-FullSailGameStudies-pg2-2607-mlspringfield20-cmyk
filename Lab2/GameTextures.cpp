...
std::map<CardSuit, pos> GameTextures::suitStarts;
...
void GameTextures::Initialize(SDL_Renderer* renderer)
{
	...
	suitStarts[CardSuit::Clubs] = { 1,1 };
	suitStarts[CardSuit::Spades] = { 1,99 };
	suitStarts[CardSuit::Hearts] = { 1,197 };
	suitStarts[CardSuit::Diamonds] = { 1,295 };
}
...
void GameTextures::RenderImage(std::string face, CardSuit suit, int x, int y, float scale, SDL_Rect* clip, double angle, SDL_Point* center, SDL_RendererFlip flip)
{
...
void GameTextures::DrawText(std::string message, int topX, int topY, int width, int height, SDL_Color color)
{
	if (font_ != nullptr)
	{
		SDL_Surface* surface = TTF_RenderText_Solid(font_, message.c_str(), color);
		if (surface != nullptr)
		{
			...
			if (texture != nullptr)
			{
				...
				dstRect.w = textW;
				dstRect.h = textH;
				dstRect.x = topX - (textW / 2);// (width - textW) / 2;
				dstRect.y = topY - (textH / 2);// (height - textH) / 2;
